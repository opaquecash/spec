...
1. **Native L1→L2 messaging** for Ethereum-origin data: `sendMessageToL2` from the Ethereum registry/ONS contracts; sequencer-automated, minutes-scale, no additional validator set. Ethereum acts as hub; Solana-origin data reaches Starknet transitively via the existing Solana→Ethereum Wormhole leg (trust is then Wormhole *plus* L1→L2 — unioned, not improved; the trust win is scoped to Ethereum-origin data such as the canonical ONS registry). Preserve the mirror invariants: emitter allowlist, monotonic sequence floor, revoke-as-tombstone (OPQ-004).
   ...
   > **Status.** The Starknet consumer (`opaquecash/starknet`, `contracts/ons_mirror`, `OpaqueNameMirror`) is built and unit-tested: an `#[l1_handler]` receives the mirror payload, the sequencer-injected L1 sender is checked against an admin-set emitter allowlist, sequences are monotonic (floor bypassed only for a genuinely new name), and a revoke tombstones in place (keys zeroed, floor retained) per OPQ-004 — mirroring the Solana `ons_mirror` semantics ([ONS.md](./ONS.md) §3) over native messaging instead of Wormhole. Declared on Sepolia (class hash `0x01d0821469d516a81cddfd6ee6d88b5073657db561d9de2f6e723649ad4ff14a`). Remaining: the Ethereum-side sender (an `OpaqueNameRegistry` hook or a mirror-sender that calls `sendMessageToL2`) and the live end-to-end delivery.
