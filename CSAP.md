...
Expected-output schema (one object per `test/test_vectors.json` entry in `opaquecash/circuits`):

```json
{
  "description": "scheme-1 secp256k1 DKSAP round trip",
  "viewing_private_key":  "0xaa..aa",
  "spending_private_key": "0xbb..bb",
  "ephemeral_private_key":"0xcc..cc",
  "viewing_public_key":   "<33-byte compressed, GENERATE>",
  "spending_public_key":  "<33-byte compressed, GENERATE>",
  "meta_address":         "<66-byte V‖S, GENERATE>",
  "ephemeral_public_key": "<33-byte compressed, GENERATE>",
  "shared_secret":        "<33-byte compressed point, GENERATE>",
  "s_h":                  "<32-byte Keccak256(shared_secret), GENERATE>",
  "view_tag":             "<s_h[0], GENERATE>",
  "stealth_address":      "<20-byte EVM-style, GENERATE>",
  "one_time_private_key": "<32-byte p_stealth, GENERATE>"
}
```

> The `GENERATE` outputs MUST be produced by running the reference implementations and cross-validated so that the Rust scanner, the TypeScript SDK (`@noble/curves`), and an independent `py_ecc` script all agree. Until cross-validated, this section is normative for *inputs and procedure* only. (Generation + cross-validation is tracked as Task 0.5 in the execution plan.)
