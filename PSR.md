...
> **Field encoding (normative).** Every public signal is a BN254 scalar-field element and MUST be reduced into `[0, r)` before it is used as a proof input or submitted on-chain — the witness signal, the on-chain public input, and any Poseidon preimage over it MUST be the identical reduced value. In particular a scope-derived `external_nullifier = keccak256(scope) mod r` and a `attestation_id` derived from a SHA-256 `schema_id` are reduced at a single source of truth; passing the un-reduced 256-bit value makes the snarkjs-generated verifier's `checkField` reject it. Both chains carry `attestation_id`/`external_nullifier` as full 32-byte big-endian field elements (Ethereum `uint256`, Solana `[u8; 32]`), so any canonical SHA-256/Keccak-derived id is representable.
...
- `verify_reputation(proof_a[64], proof_b[128], proof_c[64], root, attestation_id: [u8; 32], external_nullifier: [u8; 32], nullifier_hash)` — REQUIRE the root entry is ≤ `ROOT_EXPIRY_SECS = 3600` old; CPI into `groth16_verifier::verify_proof_v2(A, B, C, {merkle_root, attestation_id, external_nullifier, nullifier_hash})`; on success initialize a `NullifierEntry` at seeds `["nullifier", nullifier_hash]` (the `init` fails if it already exists → one-time use); emit `ReputationVerified`. `attestation_id`/`external_nullifier` are 32-byte big-endian field elements (they were `u64`, which capped the domain at 2^64 — OPQ-008). `initialize` is gated to the program's upgrade authority (OPQ-007).
...
**Ethereum — `OpaqueReputationVerifierV2.sol` + `Groth16VerifierV2.sol`:** perform the V2 check against the four V2 public signals via the `ecPairing` precompile (`0x08`, EIP-197) and `ecAdd`/`ecMul` (`0x06`/`0x07`, EIP-196), with an equivalent root-history + nullifier registry. Schemas and attestations live in `OpaqueSchemaRegistry.sol` / `OpaqueAttestationRegistry.sol` (block numbers stand in for slots). An optional admin-set schema-registry binding requires the proof's `attestation_id` to reference a **live registered schema** before the pairing (OPQ-006), so a proof cannot assert reputation under a never-registered schema; binding the **issuer** (that an authorized key attested) additionally requires committing the schema authority in-circuit and is the tracked full remediation.
